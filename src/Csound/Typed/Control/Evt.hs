...
saveEvtInstr :: Arg a => Int -> InstrId -> Evt (D, D, a) -> GE [Sig]
    ...
    where
        ...
        go mchnId es = 
            runEvt es $ \(start, dur, args) -> fromDep_ $ do
                chnId <- mchnId
                e <- C.Event instrId <$> toGE start <*> toGE dur <*> (fmap ((++ [C.chnRefId chnId]) . fmap prim) $ toNote args) 
                return $ C.event e 
